...
//     }
//     else{
//         auton_azim_kP = 0.5; //azimuth, for correcting rotation
//         auton_azim_kI = 0.0;    //drunk
//         auton_azim_kD = 10.0;
...
//             errorX = 0.0;
//         if(fabs(global_distX) > fabs(targetX))
//             errorX = 0.0;
...
//         target_v = normalizeJoystick(check_sign(targetX)*target_v_x, check_sign(targetY)*target_v_y).scalar(MAX_SPEED*0.6); // target velocity
...
//         // calculate the error angle 
//         if(target_v.norm()>0.005 || target_r.norm()>0.005){
//             l_error = angle(v_left, current_left_vector); 
//             r_error = angle(v_right, current_right_vector); 
//         }else{ //NOT TL NOT ROTATE
//             l_error = 0.0;  //DO NOT CHANGE WHEEL ANGLE IF NOT MOVING WHEEL
...
// //     bool tare_offsetX = true;
// //     bool tare_offsetY = true;

// //     while(true){
// //         if(fabs(auton_target_x) > 0.0)
// //             if(tare_offsetX == true){
// //                 offsetX = fabs(global_distX);
// //                 tare_offsetX = false;
// //             }
...
// //             if(tare_offsetY == true){
// //                 offsetY = fabs(global_distY);
// //                 tare_offsetY = false;
// //             }
...
// //         if(fabs(auton_target_heading) <= 0.0 || fabs(errorheading) <= 2.0 || fabs(errorheading) > fabs(auton_target_heading))
// //             errorheading = 0.0;
...
//     auton_heading_kP = 0.16;
//     auton_heading_kI = 0.0;
//     auton_heading_kD = 0.4;
...
//     pros::Task slam_dunk(slamDunk); //Used for both auton and driver
// }
...
//     offsetX = fabs(global_distX);
//     while(true){
//         if(fabs(fabs(global_distY) - offsetY) >= 5.0 || fabs(fabs(global_distX) - offsetX) >= 5.0){
//             pros::lcd::print(3,"left max: %d", max_4(luA.get_voltage(),luB.get_voltage(),llA.get_voltage(),llB.get_voltage()));
//             pros::lcd::print(4,"right max: %d", max_4(ruA.get_voltage(),ruB.get_voltage(),rlA.get_voltage(),rlB.get_voltage()));
//             pros::lcd::print(6,"MAXPOWER: %d",power);
//             power = 0;
//             offsetY = fabs(global_distY);
//             offsetX = fabs(global_distX);
//         }
//         move_voltage_wheels(power, power, power, power);
//         power += 30;
//         pros::lcd::print(5, "power: %d", power);
