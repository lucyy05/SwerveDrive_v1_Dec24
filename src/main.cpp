...
void pivotWheels(double l_target_angle, double r_target_angle, double allowed_error) 
//pivot the left and right wheels by a certain amount
{
    ...
    while(fabs( l_angle_error) > allowed_error || fabs( r_angle_error) > allowed_error){ 
        //while we havent reached the target pivot angles
        ...
        //note that the function getNormalizedSensorAngle already implements wrapAngle to bound the angle between -180 and 180 degrees
...
void move_auton()
{
    ...
    rotateWheels(500, 500, 10); //move forward 1000mm
    pivotWheels(0, 0, 0.1); //point both wheels forwards
}
