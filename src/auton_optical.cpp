...
void update_turning(double kP, double kD) {
  if (is_turning) {
    ...
    luA.move(power); // Adjust left side for turning
    luB.move(power);
    llA.move(power);
    llA.move(power);
    ruA.move(-power); // Adjust right side for turning
    ruB.move(-power);
    rlA.move(-power);
    rlB.move(-power);
